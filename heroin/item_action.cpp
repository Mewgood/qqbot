...
void d2_client::item_action(std::string const & data)
{
	...
	*/
	...
	if (item.ground || item.action == item_action::add_to_shop)
		items[item.id] = item;

	// Hacky pickit
	/*if (item.ground) {
		std::cout << "Quality:" << item.quality << ", code:" << item.type << ", playerx:" << me.location.x << ", playery:" << me.location.y << ", item.x:" << item.x << ", item.y:" << item.y << std::endl;
		if (item.type == "mp5") {
			d2_client::move_to(coordinate::coordinate(item.x, item.y));
			nil::sleep(300);
			send_packet(std::string("\x16\x04\x00\x00\x00", 5) + dword_to_string(item.id) + std::string("\x00\x00\x00\x00", 4));
			std::cout << "Pick'd!" << std::endl;
		}
	}*/
...
item_type d2_client::parse_item(std::string const & data)
{
	...
	try
	{
		...
		//std::cout << std::hex << item.action << std::dec << ", ";
