...
ulong d2_client::get_npc(std::string name) {
	for (std::map<unsigned, npc_type>::iterator it = npcs.begin(); it != npcs.end(); ++it) {
...
ulong d2_client::get_alive_npc(std::string name, double range) {
	for (std::map<unsigned, npc_type>::iterator it = npcs.begin(); it != npcs.end(); ++it) {
		if (it->second.name == name && it->second.life > 0x00 && (range == 0 || range > me.location.distance(it->second.location))) {
			return it->first;
		}
	}
...
ulong d2_client::get_range_npc(double range) {
	for (std::map<unsigned, npc_type>::iterator it = npcs.begin(); it != npcs.end(); ++it) {
		if (it->second.life > 0x00 && range > me.location.distance(it->second.location) && it->second.name != "Guard") {
			return it->first;
		}
	}
...
void d2_client::npc_assignment(std::string const & data)
{
	...
	try
	{
		...
		if(data.size() > 0x10)
		{
			...
			// Hacky fix for some weird bitreader bug
			if (data.size() > 17) {
				while(true) {
